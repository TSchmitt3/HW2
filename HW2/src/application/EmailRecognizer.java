	...
	// console test
		public static void main(String[] args) {
			String[] shouldBeValid = {
					"name@example.com",
					"tyler.schmitt@asu.sundevils.edu", 
					"this-email@email.net",
					"1@2.org"
			};
			
			String[] shouldBeInvalid = {
					"name@.com",				//empty domain
					".test@email.com",			//starts with .
					"this-@domain.net",			//ends with -
					"double..dot@this.com",		//double period
					"a@-domain.com",			//domain starts with -
					"a@domain-.com",			//domain ends with -
					"asdf@domain.xyz",			//invalid extension
					"",							//empty
					null						//null
			};
			
			System.out.println("=====Valid Tests=====");
			for(String s : shouldBeValid) probe(s);
			
			System.out.println("\n=====Invalid Tests=====");
			for(String s: shouldBeInvalid) probe(s);
		}

		private static void probe(String s) {
			String msg = validate(s);
			System.out.printf("%-35s -> %s%n", String.valueOf(s), (msg.isEmpty() ? "VALID" : "INVALID: " + msg));
		}
}
